...
const asgn= Symbol(":")
const prmp= x=> !udfp(x)&&x.t===prm
const resp= x=> !udfp(x)&&x.t!==prm                                                                                         // resolved

const gfn= (x,y)=> x.t=="vb"? vbs[x.v][y]:
	x.t="e"? gfn(x.v,y): x;
const tev2= (c,f,x,y)=> {
	if (f===asgn) {ctx[x.v]=bd(y); return bd(y)};
    return [x.r({x:2}),y,c];                                                                                                // dbg
    x=resp(x)? x: (!udfp(x)&&c[x.v])??x.r(c);
    y=resp(y)? y: (!udfp(y)&&c[y.v])??y.r(c);
	return resp(y)?
	f.t==="drv"? f.v(x)(y): f[1](x,y):
    {t:prm, r:(sc=>tev2(sc,f,x,y))}
                                                                                                                            // {t:"dyad", f: (arrp(f)? f[1]: f.v), x, y:bd(y)}
}
const tev1= (c,f,x)=> {
  if (f===asgn) return x
  x=resp(x)? x: c[x.v]??x;
  x=!udfp(x)&&x.t===prm? x.r(c,f,x): x;
  return resp(x)?
  f.t==="drv"? f.v()(x): f[0](x):
  {t:prm, r:(sc=>tev1(sc,f,x))}
...
e=	x:n _ f:v _ y:e {return tev2(ctx,f,x,y)}
	/ h:n _ tl:e {return udfp(tl)? h: {t:"monad", h, tl}}
	/ h:v _ tl:e {return udfp(tl)? h: tev1(ctx,h,tl)}
    / _ {return undefined}
...
Nm=	L(L/D)* {let v=text(); return !udfp(ctx[v])? ctx[v]: {t:prm, r:c=>c[v], v}}
St=	'"'e:Cs'"' {return bd(e)} 
	/"0x"e:Bs {return bd(e)}
